...
pub async fn declare() -> Result<()> {
    ...
    let contract_artifact: SierraClass = serde_json::from_reader(
        std::fs::File::open("./../target/dev/compiled_TokenRemover.sierra.json").unwrap(),
    )
    .unwrap();
    ...
    let casm_artifact: CompiledClass = serde_json::from_reader(
        std::fs::File::open("./../target/dev/compiled_TokenRemover.casm.json").unwrap(),
    )
    .unwrap();
